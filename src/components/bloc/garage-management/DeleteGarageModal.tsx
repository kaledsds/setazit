...
export const DeleteGarageModal = ({
  garageId,
  garageName,
}: DeleteGarageModalProps) => {
  const [open, setOpen] = React.useState(false);
  const utils = api.useUtils();

  const deleteMutation = api.garage.delete.useMutation({
    onSuccess: async () => {
  ...
  const handleDelete = () => {
    deleteMutation.mutate({ id: garageId });
  };
      ...
      <DialogContent className="bg-card-car rounded-xl border border-[rgba(212,175,55,0.3)] shadow backdrop-blur">
        <DialogHeader>
          ...
          <p className="text-foreground pt-4">
            ...
            <span className="font-semibold text-yellow-500">{garageName}</span>{" "}
            ?
            <br />
